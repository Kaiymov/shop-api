...
class ActivationView(APIView):
    ...
    def get(self, request, activation_code):
        try:
            ...
            user.save()
            return Response({
                'msg': 'Successfully acivated'}, status=200,
            )
...
class ForgotPasswordView(APIView):
    permission_classes = (permissions.AllowAny,)
    def post(self, request):
        serializer = serializers.ForgotPasswordSerialzer(data=request.data)
        serializer.is_valid(raise_exception=True)
        try:
            ...
            return Response('Cheack your mail!', status=200)
...
class RestorePasswordView(APIView):
    ...
    def post(self, request):
        serializer = serializers.RestorePasswordSeializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
